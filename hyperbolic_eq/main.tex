...
\[
  \pdv{\rho u_i} + \pdv{\rho u_i u_j}{x_j} = -\pdv{p}{x_i}.
\]
...
Now we can use integration by parts\footnote{%
  A quick reminder:
  \[
    \int\limits_a^b \odv{u(x)}{x} v(x) \d x = 
    uv|_a^b
    - \int\limits_a^b u(x)\odv{v(x)}{x}  \d x ,
  \]
  \[
...
\begin{equation*}
  \begin{split}
    ...
    &+ \int\limits_{0}^{\infty} \underbrace{f(\infty, t)\Phi(\infty, 0)}_{=0} \d t
    - \int\limits_{0}^{\infty} \underbrace{f(-\infty, t)\Phi(-\infty, 0)}_{=0} \d t.
  \end{split}
\end{equation*}
