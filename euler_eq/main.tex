...
\title{Euler equations \& linear hyperbolic systems}
...
(strong form):\sidenote{%
	We make several assumptions. These equations describe the compressible flow of
	an inviscid fluid without heat transfer. The symbols are:
	\begin{description}
		\item[$x_i, t$] spatial and temporal coordinates
		\item[$\rho$] fluid's density,
		\item[$u_i$] fluid's velocity vector
		\item[$p$] pressure field
		\item[$f_i$] local acceleration (e.g., gravity field)
		\item[$E$] total energy (sum of internal energy $e$ and kinetic energy $u_i u_i/2$)
		\item[$Q$] local energy source.
	\end{description}
}
\begin{equation}
	\begin{split}
		\text{conservation of mass:}\quad & \pdv{\rho}{t} + \pdv{\rho u_i}{x_i} = 0,    \\
		\text{cons.\ of momentum:}\quad   & \pdv{\rho u_i}{t} + \pdv{\rho u_i u_j}{x_j}
		= -\pdv{p}{x_i} + \rho f_i,                                                     \\
		\text{cons.\ of energy:}\quad     & \pdv{\rho E}{t} + \pdv{\rho E u_j}{x_j}
		=  -\pdv{u_i p}{x_i} + \rho f_i u_i + Q
	\end{split}
	\label{eq:euler}
\end{equation}

We can get the integral (weak) form of the equations by integrating in some
volume \(V\) with boundary \(S\). Taken continuity equation (mass conservation law)
as an example, we first integrate
\[
	\int\limits_V \pdv{\rho}{t} \d V
	+ \int\limits_V \pdv{\rho u_i}{x_i} \d V = 0,
\]
than we assume that the volume does not change in time, so we can get the time
derivative out
\[
	\pdv{}{t}\int\limits_V \rho \d V
	+ \int\limits_V \pdv{\rho u_i}{x_i} \d V = 0,
\]
and finally we use divergence theorem\sidenote{%
	Assuming that V is a closed volume with piecewise smooth boundary $S$ and $n_i$ is
	the outward pointing normal vector of $S$, we can write
	\[
		\int\limits_V \pdv{k_i}{x_i} \d V = \oint\limits_S k_i n_i \d S
	\]
	for continuously differentiable vector $k_i$.
} of the second term to get:
\[
	\pdv{}{t}\int\limits_V \rho \d V
	+ \oint\limits_S\rho u_i n_i \d S = 0.
\]

The weak form looks as follows (we moved some terms around):
\begin{equation}
	\begin{split}
		 & \pdv{}{t}\int\limits_V \rho \d V
		+ \oint\limits_S\rho u_i n_i \d S = 0,  \\
...
\end{equation}
We have four unknowns \(\rho, \bm{u}, E, p\) (we assume that \(f = Q = 0\) or
are known) but we have only three equations, but turns out we can eliminate pressure under
certain assumptions.
For an ideal calorically perfect
gas, we can write:\sidenote{%
We start from the ideal gas equation
\[
	\frac{p}{\rho} = \mathcal{R}T
\]
combined with relationships $\mathcal{R}=c_p - c_v$, $\gamma={c_p}/{c_v}$ and the formula for
internal energy $e=c_vT$.
}
\begin{equation}
	p = \rho e \ab(\gamma - 1)
	\rightarrow
...
If we consider a variable \(y = p/\rho^\gamma\), so that entropy is given as $S=c_v\ln y + \text{const.}$, we can compute the expression
\[
	\pdv{y}{t} = \frac{1}{\rho^\gamma}\pdv{p}{t} + \pdv{\rho^{-\gamma}}{t}p
	= \frac{1}{\rho^\gamma}\pdv{p}{t} - \gamma p \rho^{-\gamma - 1}\pdv{\rho}{t}
	= \frac{1}{\rho^\gamma}\ab(\pdv{p}{t} - \frac{\gamma p}{\rho}\pdv{\rho}{t})
\]
which can be further simplified using the speed of sound, \nth{1} and
\nth{3} equation in the primitive form given by \cref{eq:primitive}, resulting in
\[
	\begin{split}
		\frac{1}{\rho^\gamma}\ab(\pdv{p}{t} - a^2\pdv{\rho}{t})
		 & = \frac{1}{\rho^\gamma}\ab[-\rho a^2 \pdv{u}{x} - u\pdv{p}{x} + a^2\ab(u\pdv{\rho}{x} + \rho\pdv{u}{x}) ] \\
		 & = \frac{-u}{\rho^\gamma}\ab( \pdv{p}{x} - a^2\pdv{\rho}{x} ) = -u\pdv{y}{x}
	\end{split}
\]
Equating one to the other gives us the equation for $y$ that is valid in
smooth regions of the flow
\[
	\pdv{y}{t} + u\pdv{y}{x} = \mdv{y}{t}=0,
\]
