...
% ------------------------------------------------------------------------------
\title{Derivation of fluid flow conservation laws\\
...
We will consider a fluid as a continuous substance, that is, our considered
fluid fills fully available space. It is important to note that, effectively,
\emph{we are neglecting the discrete nature of matter.} We can do that, since
the amount of fluid molecules in small volume of matter is obscenely large
(remember the Avogadro number \(N_a\approx\SI{6.023e26}{kmol^{-1}}\)).
...
the trajectories, we also encounter a problem of determining an accurate
initial conditions for every molecule. In such systems, a small error in
determining these starting parameters will unfortunately lead to considerable
inaccuracies further down the line.

Hence, we neglect the discrete nature of matter and we imagine that the medium,
i.e.\ the fluid, consists of continuous set of ideal particles%
\footnote{%
...
So, the movement of the medium is \emph{de facto} the movement of these points,
and it turns out that such a description can be used for fluids, solids and
\say{things} in between. In comparison to solids, which have some \say{rigid}
reference configuration or shape, fluids do not exhibit this property. The
relative positions of the points can be arbitrary and changing. Naturally,
while approaching a fluid mechanics problem, we have to make certain
...
of the fluid can be completely arbitrary. \emph{Water spilled on the floor and
left to its own devices, will deform greatly, covering the area and even
seeping into the cracks, potentially making you call the handyman to fix the
broken flooring.} So what creates the internal forces inside the fluid? One
obvious answer is the change of volume, but what else?

We will consider fluids in which the forces depend on the rate of deformation.
...
does not seem to be significant. However, it is important to note, that this is
only a simplified model, not truly existing in nature. In the same way as the
assumption of incompressibility, which is never true, as each fluid is
compressible, it is a useful lie that does a good job describing the reality as
we can measure it. For the most general fluid we could write
\[
...
\(\bm{\tau}\), deformation \(\bm{D}\), the rates of those things (indicated by
...
\begin{enumerate}[(i)]
  ...
  \item we should be able two solve for \(\bm{\tau}\), that is write 
    \(\bm{\tau} = g(\dot{\bm{D}})\) where \(g\) is some function with tensor argument;
...
differently than the same fluid moving along \(y\) axis. In practice, this will
require that the \(\bm{\tau}\) and \(\dot{\bm{D}}\) are, among other things,
\emph{symmetric}.
